...
class AddRecipePage extends StatefulWidget {
  const AddRecipePage({super.key, this.initialFolderId});

  /// When set to a user folder (e.g. from a folder page), pre-selects that folder.
  /// When null/omitted or Recent, defaults to Recent recipes.
...
class _AddRecipePageState extends State<AddRecipePage> {
  ...
  void initState() {
    ...
    _folderId = (initial == null || initial == RecipeStore.recentFolderId)
        ? RecipeStore.recentFolderId
        : initial;
  }
  ...
  void _save() {
    ...
    if (name.isEmpty) {
      _showError('Recipe name is required');
      return;
    }
    ...
    if (_preparationMinutes == null || _preparationMinutes! <= 0) {
      _showError('Preparation time is required');
      return;
    }
    if (_folderId.isEmpty) {
      _showError('Recipe folder is required');
      return;
    }
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text('Add Recipe', style: titleStyle),
                  ),
                  IconButton(
                    onPressed: _save,
